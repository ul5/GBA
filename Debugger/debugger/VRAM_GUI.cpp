...
void Debugger::VRAM_GUI::render() {
    ...
    for(int index = 0; index < 128 && !(dispcnt & 0x80); index++) {
        ...
        //else if(rotscal) printf("Rotation and scaling apply here\n");
                ...
                for(int j = 0; j < 8; j++) {
                    for(int i = 0; i < depth; i++) {
                        ...
                        if(depth == 4) {
                            ...
                            SDL_SetRenderDrawColor(renderer, (color & 0x1F) << 3, ((color >> 5) & 0x1F) << 3, ((color >> 10) & 0x1F) << 3, 0xFF);
                            if(color) SDL_RenderDrawPoint(renderer, (int) x + xc * 8 + i * 2 + 0, (int) y + yc * 8 + j);
                            color = palette_obj[b];
                            SDL_SetRenderDrawColor(renderer, (color & 0x1F) << 3, ((color >> 5) & 0x1F) << 3, ((color >> 10) & 0x1F) << 3, 0xFF);
                            if(color) SDL_RenderDrawPoint(renderer, (int) x + xc * 8 + i * 2 + 1, (int) y + yc * 8 + j);
                        } else {
                            hword color = palette_obj[cpu->r8(0x06010000 + offset)];
                            SDL_SetRenderDrawColor(renderer, (color & 0x1F) << 3, ((color >> 5) & 0x1F) << 3, ((color >> 10) & 0x1F) << 3, 0xFF);
                            if(color) SDL_RenderDrawPoint(renderer, (int) x + xc * 8 + i, (int) y + yc * 8 + j);
                        }
                    }
                }
            }
